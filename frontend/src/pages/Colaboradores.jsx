...
export default function Colaboradores() {
  ...
  const load = async () => {
    try {
      const data = await get('/api/colaboradores')
  ...
  return (
    <>
      ...
      <div className="cards-list">
          ...
          <article className="finance-card" key={item.id}>
            <div>
              ...
              <div className="meta-row">
                <span><Icon name="dollar" size={14} /> {item.email ? item.email.replace(/"/g, '') : ''}</span>
        ...
        {items.length === 0 && !erro && <p style={{ textAlign: 'center', color: '#888', padding: 32 }}>Nenhum colaborador encontrado</p>}
      </div>
    </>
  )
}
