...
export default function Permissoes() {
  const { user } = useAuth()
  const [usuarios, setUsuarios] = useState([]); const [recursos, setRecursos] = useState([])
  const [usuarioId, setUsuarioId] = useState(''); const [permissoes, setPermissoes] = useState([])
  const [erro, setErro] = useState(''); const [mensagem, setMensagem] = useState(''); const [saving, setSaving] = useState(false)
  ...
  const handleChange = async (e) => {
    ...
    if (id) {
      try { const res = await get('/api/recurso/' + id + '/permissoes'); setPermissoes(res.permissoes || []) }
      catch (error) { setErro(error.message) }
    }
  }
  ...
  const salvar = async () => {
    ...
    try {
      ...
      setPermissoes(res.permissoes || []); setMensagem('Permissoes atualizadas!')
    ...
    <>
      ...
      <div className="editor-card">
        <label style={{ display: 'block', marginBottom: 16 }}>
          ...
          <select value={usuarioId} onChange={handleChange} style={{ width: '100%', padding: '8px 10px', border: '1px solid #d9e0ea', borderRadius: 4, fontSize: 13 }}>
        ...
        {usuarioId && (
          <>
            <div className="perm-panels">
              <div className="perm-panel" style={{ borderTop: '3px solid #23598d' }}>
                <div className="perm-panel-header"><h3>Recursos do Usuario <small>({recursosDoUsuario.length})</small></h3></div>
                <div className="perm-panel-body">
                  {recursosDoUsuario.length === 0 ? <div className="perm-empty">Nenhum</div> : recursosDoUsuario.map(r => (
                    <div className="perm-item" key={r.id}>
                      <div><div className="perm-item-name">{r.nome}</div><div className="perm-item-desc">{r.descricao}</div></div>
                      <button className="icon-button" title="Remover" onClick={() => setPermissoes(prev => prev.filter(p => p !== r.nome))}><Icon name="trash" size={14} /></button>
                    </div>
                  ))}
                </div>
              </div>
              <div className="perm-panel" style={{ borderTop: '3px solid #27ae60' }}>
                <div className="perm-panel-header"><h3>Disponiveis <small>({recursosDisponiveis.length})</small></h3></div>
                <div className="perm-panel-body">
                  {recursosDisponiveis.length === 0 ? <div className="perm-empty">Nenhum</div> : recursosDisponiveis.map(r => (
                    <div className="perm-item" key={r.id}>
                      <div><div className="perm-item-name">{r.nome}</div><div className="perm-item-desc">{r.descricao}</div></div>
                      <button className="icon-button" title="Adicionar" onClick={() => setPermissoes(prev => [...prev, r.nome])}><Icon name="plus" size={14} /></button>
                    </div>
                  ))}
                </div>
              </div>
