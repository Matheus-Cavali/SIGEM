...
import org.example.controller.LocalEventoControl;
...
public class LocalEventoRouter implements HttpHandler {

    private static LocalEventoControl control;
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if("OPTIONS".equalsIgnoreCase(metodo)){
            ...
            if("POST".equalsIgnoreCase(metodo) && "/api/locais-evento".equals(path)){
                String json = new String(exchange.getRequestBody().readAllBytes(), StandardCharsets.UTF_8);
                r = getControl().cadastrar(auth, json);
            }
            else if("GET".equalsIgnoreCase(metodo) && "/api/locais-evento".equals(path)){
                r = getControl().listar(exchange.getRequestURI().getQuery());
            }
            else if("PUT".equalsIgnoreCase(metodo) && path.matches("/api/locais-evento/\\d+")){
                int id = Integer.parseInt(path.substring(path.lastIndexOf("/") + 1));
                String json = new String(exchange.getRequestBody().readAllBytes(), StandardCharsets.UTF_8);
                r = getControl().atualizar(auth, id, json);
            }
            else if("DELETE".equalsIgnoreCase(metodo) && path.matches("/api/locais-evento/\\d+")){
                int id = Integer.parseInt(path.substring(path.lastIndexOf("/") + 1));
