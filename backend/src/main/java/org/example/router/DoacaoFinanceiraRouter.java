...
public class DoacaoFinanceiraRouter implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if("OPTIONS".equalsIgnoreCase(metodo)){
            ...
            else if("PUT".equalsIgnoreCase(metodo) && path.matches("/api/doacoes-financeiras/\\d+")){
                int id = Integer.parseInt(path.substring(path.lastIndexOf("/") + 1));
                String json = new String(exchange.getRequestBody().readAllBytes(), StandardCharsets.UTF_8);
                r = getControl().atualizar(auth, id, json);
            }
            else if("DELETE".equalsIgnoreCase(metodo) && path.matches("/api/doacoes-financeiras/\\d+")){
                int id = Integer.parseInt(path.substring(path.lastIndexOf("/") + 1));
